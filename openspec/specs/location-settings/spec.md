# location-settings Specification

## Purpose
The location-settings capability lets the user choose and persist the location that other panels report on. It covers resolving a user-entered place name to coordinates via the Open-Meteo geocoding API, presenting and disambiguating candidates, persisting the selected location to a configuration file under the user's config directory, the shipped default used before any selection is made, and the fallback behaviour when the configuration file cannot be read or written.
