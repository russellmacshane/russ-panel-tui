...
## Purpose
The release-automation capability governs how a version of the package is published from continuous integration rather than from a maintainer's machine: what triggers a release, how the tag and the manifest version are kept in agreement, the test-suite gate a release must pass, the credential-free identity exchange that authenticates the publish, the isolation of that identity between jobs, the toolchain floor trusted publishing requires, the provenance attestation a release carries, and the registered workflow filename the registry's trust configuration depends on.
